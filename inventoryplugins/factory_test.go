...
func TestNew(t *testing.T) {
	...
	if _, err := inventoryplugins.GetPlugin("test", &config.MachineInventoryPluginSettings{}, &config.Config{}, func(s string, i config.LogLevel) bool { return true }); err == nil {
		t.Errorf("Plugin factory did not return error for unknown type.")
	}

	if err := inventoryplugins.AddMachineInventoryPlugin("test", func(s *config.MachineInventoryPluginSettings, c *config.Config, lf func(string, config.LogLevel) bool) inventoryplugins.MachineInventoryPlugin {
		return &TestPlugin{}
	}); err != nil {
