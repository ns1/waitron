...
func TestNew(t *testing.T) {
	...
	if _, err := inventoryplugins.GetPlugin("test", &config.MachineInventoryPluginSettings{}, &config.Config{}); err == nil {
		t.Errorf("Plugin factory did not return error for unknown type.")
	}

	if err := inventoryplugins.AddMachineInventoryPlugin("test", func(s *config.MachineInventoryPluginSettings, c *config.Config) inventoryplugins.MachineInventoryPlugin {
		return &TestPlugin{}
	}); err != nil {
